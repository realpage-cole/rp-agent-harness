...
**A local multi-agent harness for [Claude Code](https://claude.com/claude-code).** `rp-agent-harness` is an internal RealPage desktop app (Electron + React + TypeScript) that turns the `claude` CLI sessions you already run in your terminal into a self-coordinating team. Each agent runs as a real Claude Code process in a pseudo-terminal, gets long-term memory and a mailbox, and is coordinated by a **god orchestrator agent** you talk to — including remotely from your phone via Claude Code's `/remote-control`, where the orchestrator surfaces any questions inline. A modern Hive dashboard shows the agent roster, a live activity feed, and a task board. You can run **several independent teams in parallel** — each its own orchestrator, hive, telemetry, and dashboard view — and **clone** an existing team's agent lineup into a fresh team with one click. Everything is local-first; an optional Supabase layer lets teammates share long-term memory and a team blackboard, and view each other's roster + task board read-only.
...
- **Orchestrator + hive coordination.** The on-disk "hive" is a single-committer, git-backed coordination layer: per-agent identity and long-term memory, atomic-file mailboxes (`outbox/` → routed into `inbox/`), a shared blackboard, and an append-only event log. A god orchestrator agent (id `god`, role `orchestrator`) adjudicates traffic, routes and assigns tasks, and marks human-blocking cards `blocked` (with the ask recorded on the card) — you answer the orchestrator directly, in person or via Claude Code remote-control. No agent ever touches git directly, which avoids `.git/index.lock` corruption.
...
- **Modern Hive dashboard.** The main view composes an agent roster, a live activity feed, and a dependency-aware kanban task board, with quick-nav into the Command Center tabs (Tasks, Schedules), plus a team selector and per-team actions (clone, clear work history, delete) when more than one team exists. All bindings — tasks, messages, agent status — are real, live data.
...
- **Renderer (React).** The modern **Hive dashboard** (`src/renderer/src/components/dashboard/*`): an agent roster, a live activity feed, and a dependency-aware kanban task board (blocked cards show what they're waiting on). It also hosts the per-agent terminals (xterm.js over the PTY bytes). It is a pure view over state the main process owns.
...
- **On-disk layout (no-move).** The default team is the legacy hive at `<harnessHome>/hive/` — it is **never relocated**. Each cloned team lives at `<harnessHome>/teams/<id>/hive/`, an independent git repo with the same layout. Feeding each `HiveManager` a team-scoped home means every root-relative path (registry, board, tasks, log, hook socket) becomes team-correct automatically.
...
- **Clone Team.** Cloning copies the source team's per-agent configs — id, name, provider, role, capabilities, working dir, custom prompt — then **starts fresh**: each agent gets a new empty `memory.md`, and the new team gets a fresh `board.md`, empty `tasks.json`, empty inboxes, a fresh `git init`, and its own `syncWorkspaceId`. No live state, history, or in-flight messages carry over. If any step fails the partially-created team is rolled back. Exposed over IPC as `teams:list` / `teams:clone` / `teams:remove` with a `teams:event` push channel. **Deleting** a team (non-default only) stops it, kills its PTYs, and erases its on-disk subtree; a per-team **clear work history** (`hive:clearHistory`) resets tasks/board/pulse/log while keeping the roster, agent memory, and cost ledger. Both live behind a confirm dialog in the dashboard's per-team actions menu.
...
- **Driving the orchestrator.** You talk to each team's god directly in its own Claude Code session — locally in the dashboard terminal, or remotely from your phone via Claude Code's `/remote-control`. There is no external message ingress; all input reaches the orchestrator through its own session.
...
A header row provides quick-nav buttons (**Tasks**, **Schedules**) into the **Command Center** tabs — the per-agent control surface for tasks and scheduled missions.
...
`rp-agent-harness` is a local-first desktop app. It spawns local processes in PTYs and reads/writes files only under directories you register, sandboxed and path-validated in the main process. The renderer has no direct Node access (`nodeIntegration: false`, `contextIsolation: true`); all `fs:*` / `git:*` IPC is typed through a `contextBridge` (`window.cth`) and rooted at an agent's working directory. The hive commits to a local git repo from a single committer (the main process); agents only write plain files. The only network listeners are local: a per-team socket (or named pipe on Windows) for the hook server, and each team's telemetry collector on an ephemeral **loopback-only** (`127.0.0.1`) port. There is no inbound network ingress — you drive the orchestrator through its own Claude Code session.
